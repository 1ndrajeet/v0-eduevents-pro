...
export function EventDashboard() {
  ...
  return <div className="min-h-screen bg-background text-foreground">
    <aside className={`fixed inset-y-0 left-0 z-50 flex w-64 flex-col border-r bg-card p-4 transition-transform md:translate-x-0 ${mobileNav ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <div className="mt-auto space-y-1 border-t pt-4"><button onClick={() => notify('Support center opened')} className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-muted-foreground hover:bg-muted"><CircleHelp size={18} />Support</button><button onClick={() => notify('You are already signed out')} className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-muted-foreground hover:bg-muted">Sign out</button></div>
    </aside>
